...
// Validate at compile-time that 'TuxFamily' implements 'Downloader'.
var _ client.Downloader[artifact.Hosted] = &TuxFamily{} //nolint:exhaustruct

/* ----------------------------- Impl: Download ----------------------------- */

// Downloads the provided asset, copying the response to all of the provided
// 'io.Writer' writers.
func (m TuxFamily) Download(a artifact.Hosted, w ...io.Writer) error {
...
func (m TuxFamily) DownloadTo(a artifact.Hosted, out string) error {
	urlParsed, err := a.ParseURL()
	if err != nil {
		return err
	}
...
func (m TuxFamily) DownloadToWithProgress(a artifact.Hosted, out string, p *progress.Progress) error {
	urlParsed, err := a.ParseURL()
	if err != nil {
		return err
	}
